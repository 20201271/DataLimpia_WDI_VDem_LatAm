...
América Latina entre 2000 y 2024.
...
|-------------------------|------------------------------------------------------------------------------------------------|
| `SI.POV.GINI`           | Gini index (medida de desigualdad de ingresos).                                               |
| `SI.POV.NAHC`           | Poverty headcount ratio at national poverty lines (% de la población).                        |
| `EN.GHG.CO2.PC.CE.AR5`  | Emisiones de CO₂ per cápita (toneladas de CO₂e).                                              |
| `ER.PTD.TOTL.ZS`        | Áreas protegidas terrestres y marinas (% del total de la superficie del país).               |
| `NY.GDP.MINR.RT.ZS`     | Renta minera (% del PIB).                                                                     |
| `NY.GDP.FRST.RT.ZS`     | Renta forestal (% del PIB).                                                                   |
| `NY.GDP.TOTL.RT.ZS`     | Renta total de recursos naturales (% del PIB).                                               |
...
| Variable       | Descripción                                                            |
|----------------|------------------------------------------------------------------------|
| `country_id`   | Identificador numérico de país en V-Dem.                               |
| `v2x_libdem`   | Índice de democracia liberal (liberal democracy index).                |
| `v2x_egaldem`  | Índice de democracia igualitaria (egalitarian democracy index).        |
| `v2x_api`      | Índice agregado de democracia/autocratización (polyarchy).             |

Los índices V-Dem están escalados aproximadamente entre 0 y 1, donde valores más
altos indican mayor nivel de democracia en la dimensión correspondiente.

---

## Cobertura geográfica

Incluye países de América Latina según disponibilidad de datos en WDI y V-Dem, por ejemplo:
...
Una forma estándar de citar WDI es:
...
En un trabajo académico en español, podrías usar:
...
V-Dem recomienda citar el dataset y el paper metodológico correspondiente.  
Un ejemplo de cita (ajusta la versión y el año a lo que uses, por ejemplo v16):
...
> [Country–Year Dataset] v16*. Gothenburg: V-Dem Institute, University of  
> Gothenburg.

En el texto puedes referirte a los índices como:

> “Índices de democracia tomados de V-Dem (Varieties of Democracy, dataset país–año, versión 16).”

Consulta siempre la página oficial de V-Dem y de WDI para confirmar la cita
recomendada para la versión concreta que utilices.
...
- Esta tabla es una integración y limpieza propia para uso académico y de
  investigación. Verifica las condiciones de uso de WDI y V-Dem si vas a
  redistribuir los datos o usarlos más allá de fines académicos.
